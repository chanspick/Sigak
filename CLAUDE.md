...
- Phase H Sia v4 — `sia_session_v4.py` / `sia_prompts_v4.py` / `sia_validators_v4.py` + persona-b 톤 (`~더라구요` → `~이에요` 변경)
...
### 6.2 어미 규칙 (2026-04-27 갱신)

**허용:** "~가봐요?" / "~이신 것 같은데" / "~으시죠?" / "~이시잖아요" / "~이에요" / "~습니다"
**금지:** "~네요" / "~군요" / "~같아요" / "~같습니다" / "~것 같" / "~수 있습니다" / **"~더라구요"** (소비자 피드백 — 톤 어색)

**변경 이력:** v2.1 (4/25) `~더라구요` 허용 → v2.2 (4/27) **소비자 피드백 반영 폐기 (`4e2b225 feat(persona-b): ~더라구요 → ~이에요`)**. 리포트 본문은 `~있어요/세요` 유지 (R7).
...
| H Sia v4 | 페르소나 B + 메시지 단위 + 100% 주관식 + 14 타입 + 5 fixture (`sia_session_v4.py` 등) |
