...
# 실패 시 &&로 uvicorn 부팅 차단 → Railway restart + health check 실패로
# 조기 가시화. DATABASE_URL 은 Railway 대시보드에서 주입되며 alembic/env.py
# 가 런타임 로드. `python -m alembic` 호출로 console script PATH 이슈 방지.
