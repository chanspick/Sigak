...
# 1. FEATURE MODIFIERS
# ============================================================
# face_feature → style별 점수 가감
# mod: 기본 modifier (-0.3 ~ +0.3)
# reason: WHY 텍스트 생성용 근거 키
# conditions: 조건부 modifier (hair_volume, styling 등에 따라 변동)
...
FEATURE_MODIFIERS = {
    ...
    "short_forehead": {
        ...
        "h-b05": {"mod": -0.15, "reason": "턱 주변 무게감으로 이마 더 좁아 보임, 턱 더 커 보임"},
    ...
    "mouth_protrusion": {
        ...
        "h-f08": {"mod": -0.1, "reason": "앞머리 없으면 시선이 하관으로 쏠림, 중단발 이상에서 특히"},
        "h-b04": {
            "mod": -0.05,
            "reason": "컬감 많으면 입 부각",
            "conditions": [{
    ...
    # ── 큰 코 ──────────────────────────────────────────
    "large_nose": {
        "h-f01": {"mod": -0.15, "reason": "눈썹 가리고 코 대비시켜 코 자체를 더 부각, 투박/중성적 이미지"},
    ...
    "short_neck": {
        ...
        "h-b12": {
            "mod": +0.05,
            "reason": "레이어드로 가벼우나 C컬 레이어드 위치 주의",
            "conditions": [{
        ...
        "h-e02": {"mod": +0.15, "reason": "높게 묶으면 목선 길어보이고 답답함 해소, 날씬해 보임"},
    },
    ...
    # ── 옆광대 없음 (거의 영향 없는 feature) ──────────────
    "no_cheekbone": {
        # 레어리에서 이 유저는 옆광대 부각이 거의 없어 별도 modifier 불필요
    },
}
...
# ============================================================
# 3. CROSS EFFECTS (앞머리 × 뒷머리 상호작용)
# ============================================================
# key: (front_pattern, back_pattern) → modifier
# pattern은 style_id 또는 속성 기반 매칭
...
CROSS_EFFECTS = {
    ...
    ("h-f04", "layered"): {
        "mod": +0.05,
        "reason": "비대칭 앞머리 + 가벼운 뒷머리 = 전체적으로 시원하고 세련된 조화",
    },
}
...
def match_cross_effect(front_style, back_style, all_styles):
    """
    front_style, back_style: style dict from HAIR_STYLES
    Returns: total cross_effect modifier and reasons
    """
    total_mod = 0.0
    reasons = []
...
def match_image_preference(style_vector, user_desired_keywords):
    """
    style_vector: [0.8, 0.6, 0.2, 0.2, 0.4]
    user_desired_keywords: ["시크", "모던", "차분"]
    Returns: bonus score (0 ~ +0.15)
    """
    KEYWORD_TO_AXIS = {
        "러블리": 0, "귀여운": 0, "사랑스러운": 0, "여리여리": 0,
        "청순": 1, "깨끗한": 1, "순수한": 1, "내추럴": 1,
        "시크": 2, "도도": 2, "쿨": 2, "세련된": 2, "모던": 2, "샤프": 2,
        "우아": 3, "차분": 3, "고급스러운": 3, "클래식": 3, "페미닌": 3,
        "개성": 4, "유니크": 4, "힙한": 4, "자유로운": 4, "보이시": 4,
    }
