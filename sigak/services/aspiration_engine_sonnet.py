...
def _photo_to_content_block(photo: PhotoInput) -> dict:
    """PhotoInput → Claude API image content block (verdict_v2 동일)."""
    if photo.get("url"):
...
def _build_user_message(
    *,
    ...
    user_n = min(len(user_photos), 5)
    target_n = min(len(target_photos), 5)

    # 사진 블록 — 라벨 텍스트로 cross 구분 명확화
    if user_n > 0:
...
# Hard Rules 검증 — Sia v4 validators 재사용 (페르소나 B 검증)
_BANNED_TONE_ENDINGS = (
    "네요.", "네요!", "네요?", "군요.", "군요!", "군요?",
    "같아요.", "같습니다", "것 같", "수 있습니다",
    # Verdict 정중체 ("~합니다") 는 페르소나 B 영역에선 금지.
    # 단 단일 문장 끝 어미만 검증 — "분석합니다" 같은 명사절은 OK.
)

_BANNED_VERDICT_WORDS = ("verdict", "Verdict", "VERDICT", "판정")
_BANNED_PRAISE = (
    "매력적", "독특한", "특별한", "흥미로운", "인상적",
    "센스 있", "안목", "감각이 있",
)
_BANNED_COMMERCE = (
    "리포트로", "구독", "추천해드릴", "정리해드릴",
...
_BANNED_AXIS = (
    "shape", "volume", "age",   # 영문 raw
    "소프트", "샤프", "프레시", "성숙", "베이비", "매추어",  # 정성 라벨
)
_BANNED_TREND_META = ("[score:", "trend_id", "↘", "▲", "🔥", "→")
_BANNED_MARKDOWN = ("**", "##", "```")
...
def compose_aspiration_v2(
    *,
    ...
    logger.error(
        "aspiration v2 all retries failed user=%s last_raw=%r",
        user_id, last_raw[:200] if last_raw else "(empty)",
    )
